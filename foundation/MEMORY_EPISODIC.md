...
| 2026-07-04 | v3.1.0 Demo Controls (Phase 4 gate 2/3) | WORK COMPLETE + LIVE — awaiting GO | ~1.5–2h / ~1.5h | ~1.5h | Admin-only POST /admin/reset (typed RESET) clears reviews/audit_index/batches/idempotency via generic _clear_table (key-schema + batch-delete); immutable S3 audit untouched. Settings "Demo controls" danger zone. IAM +BatchWriteItem/+DescribeTable + idempotency wired from api_intake. pytest 39/39 console (+3), vitest 28/28 (+2), plan 0-drift. Live: guards 403/400, real reset 200/420 cleared, dashboards zero, 217 audit objects locked. 2 deploy-only IAM re-applies (DescribeTable, BatchWriteItem — moto skips IAM). |
