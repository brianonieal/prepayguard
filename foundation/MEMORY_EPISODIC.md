...
| 2026-07-04 | v3.0.0 Executive Showcase (Phase 4 gate 1/3) | CLOSED (tagged) | ~3–4h / ~1.5h | ~1.5h | New "Overview" console tab (Showcase.jsx, canReview-gated): hand-built SVG donut/gauge/timeline/match-type/pipeline-flow + decision/intelligence/trust prose + 3 live worked examples. console_api GET /showcase = shared _compute_summary + bounded 40-record match-type tally + one example per disposition (backfilled); no new IAM, no new DEC. pytest 87/87, vitest 26/26, plan 0-drift, CORS green. Live: /showcase 200, 178 screened, 3 examples resolved. Tab later moved after Review Queue. Isolated-harness visual verify. |
| 2026-07-04 | v3.1.0 Demo Controls (Phase 4 gate 2/3) | CLOSED (tagged) | ~1.5–2h / ~1.5h | ~1.5h | Admin-only POST /admin/reset (typed RESET) clears reviews/audit_index/batches/idempotency via generic _clear_table (key-schema + batch-delete); immutable S3 audit untouched. Settings "Demo controls" danger zone. IAM +BatchWriteItem/+DescribeTable + idempotency wired from api_intake. pytest 39/39 console (+3), vitest 28/28 (+2), plan 0-drift. Live: guards 403/400, real reset 200/420 cleared, dashboards zero, 217 audit objects locked. 2 deploy-only IAM re-applies (DescribeTable, BatchWriteItem — moto skips IAM). |
| 2026-07-04 | v3.2.0 Console Depth (Phase 4 gate 3/3 FINAL) | WORK COMPLETE + LIVE (committed+tagged) | ~2–3h / ~1.5h | ~1.5h | Profile rewritten real (ID-token fields + Amplify Change password + TOTP MFA enroll/disable); Login handles CONFIRM_SIGN_IN_WITH_TOTP_CODE (no lockout); Settings inert toggles removed. Cognito pool → OPTIONAL software-token MFA (in-place). No backend change. vitest 31/31 (+3), pytest 90/90, plan 0-drift, CORS green. Live: pool MFA OPTIONAL/TOTP true, SPA deployed, Profile harness-verified. Password/MFA round-trips need real browser+authenticator (enroll on non-admin acct first). **Phase 4 COMPLETE.** |
