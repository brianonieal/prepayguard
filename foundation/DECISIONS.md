...
**Alternatives considered:**
- **Cap at NACHA 22 instead of Fedwire 35** (rejected as default). The implemented control **REJECTS with 400, it does not truncate**, so the cap causes **no screening miss** — it causes a 400 on a legitimate long payee. Restated on the grounds that actually apply, verified against the live 96-entry v4 histogram (max name 66 chars): **35 chars → 11/96 legitimate names rejected, 75/96 (78%) of the list still evadable; 22 chars → 29/96 rejected, 31/96 (32%) evadable.** 35 is chosen on **operational-usability** grounds: it rejects 11 legit names instead of 29 (fewer false 400s on real long entity names), at the explicit cost that **78% of the reference list remains evadable** by an in-budget append (vs 68% closed only if you drop to 22 and reject 3× as many legit names). Neither cap is close to closing the append class; 22 remains reachable via `PAYEE_MAX_LENGTH` if a program accepts the higher legit-reject rate. **The length cap is not the load-bearing control** (see below).
- **Truncate `payee` to the cap instead of rejecting** (rejected): 2.1b showed trailing truncation is defeated by prefix/infix placement, and 2.1d(b) showed a truncating cap would introduce its own false-negative (a truncated legit long name misses its own listed entity: 8/96 at a 22-char cap, 2/96 at 35). We reject instead, which is fail-closed and has no screening-miss cost — only the availability cost above.
...
**Rationale:** repairs the input contract (the 2.1a/2.1c root cause) at the edge and in-handler, cheaply, without touching the matcher (no eval re-sweep needed). **The character-class rule is doing the heavy lifting, not the length cap:** the `^[ -~]+$` printable-ASCII pattern closes the **entire** homoglyph/transliteration class (Cyrillic and fullwidth are not printable ASCII — 2.1d(a)), whereas the 35-char cap closes only **~22% of the append class** (21 of 96 entities lack the budget for a fitting append; 75/96 keep it). Flag-gated default-ON preserves the demo's ability to show the attack. **Note on the character-class cost (C3 correction):** the rule screens the incoming **payee stream**, not the reference list, so the fact that all 96 *reference* names are ASCII is irrelevant to its cost. The false-reject lands on the **payment side**: a legitimate payee such as `José Muñoz` (diacritics), `François Lefèvre`, or any non-Latin-script name receives a **400** at intake. That cost is real and unquantified here (we have no distribution of real payee names); it is not "0/96". Do not cite the reference list's ASCII-ness as evidence the rule is cheap.
**Risk acknowledged:** (1) **KNOWN LIMITATION — this does NOT close F1.** 2.1d measured the residual: **75/96 (78%) listed entities remain evadable** via an in-budget ASCII append under a 35-char cap; the windowed matcher is the recommended backstop, not built here. (2) ASCII-printable **rejects legitimate diacritic names** (`José Muñoz`, `François`) — a real false-reject on the **payee stream** (the rule screens incoming payees, not the reference list), asserted in `test_ascii_rule_rejects_legitimate_diacritics_known_limitation`; the Latin-script+NFKC variant is the documented mitigation. The cost is unquantified (no real payee-name distribution); the reference list being all-ASCII says nothing about it. (3) Rejecting `payee` over 35 chars **400s 11 of the 96 real listed names** — including the longest real entity, `LIMITED LIABILITY COMPANY SPECIALIZED DEVELOPER ALABUGA SOUTH PARK` (66 chars); a legitimate payment carrying such a name is rejected (fail-closed, safe, but an availability cost to route out-of-band). (4) The edge `pattern` relies on API Gateway request-body validation; the in-handler check is the defense-in-depth / direct-invoke guarantee.
**Verified live (2.1f):** deployed to dev with validation ON; SigV4 `POST /dev/payments` of the longest real listed name (`LIMITED LIABILITY COMPANY SPECIALIZED DEVELOPER ALABUGA SOUTH PARK`, 66 ch) → **HTTP 400** at the edge, a Cyrillic homoglyph payee → **HTTP 400** (pattern), a clean short name → **200 queued**. So API Gateway does enforce both `maxLength` and the printable-ASCII `pattern`, and a real listed name fails our own cap (11/96). A deployment gotcha surfaced and is fixed + recorded in **ERR-1**: a Terraform `? :` between two differently-shaped schema objects stringified the integer `maxLength` (`"35"`), which API Gateway rejects; fixed by selecting between two independently-`jsonencode`d strings.
**Confidence:** HIGH (matcher unchanged; measured tradeoffs; deployed and verified). **Reversibility:** HIGH — single Terraform flag returns the prior schema and handler behavior; no data/IAM/infra migration.
**Resolution:** PROCEED
**Status:** LOCKED
