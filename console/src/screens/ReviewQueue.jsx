...
export default function ReviewQueue({ onOpen, defaultFilter = "pending" }) {
  ...
  const [window_, setWindow] = useState("all"); // all | 1 | 7 (days)

  const counts = FAKE_REVIEWS.reduce((a, r) => ({ ...a, [r.status]: (a[r.status] || 0) + 1 }), {});
  const q = query.trim().toLowerCase();
  const rows = FAKE_REVIEWS.filter((r) => {
  ...
  const pend = FAKE_REVIEWS.filter((r) => r.status === "pending");
  const avg = pend.length ? Math.round(pend.reduce((s, r) => s + r.score, 0) / pend.length) : "—";
  const oldest = pend.length ? AGE(pend[pend.length - 1].received_at) : "—";
  ...
  return (
    <div className="body">
      ...
      <div className="filters">
        {["pending", "approved", "rejected", "all"].map((f) => (
          <button key={f} className={`chip ${status === f ? "on" : ""}`} onClick={() => setStatus(f)}>
            {f[0].toUpperCase() + f.slice(1)} ({f === "all" ? FAKE_REVIEWS.length : counts[f] || 0})
          </button>
        ))}
      </div>
      ...
      {rows.length === 0 ? (
        ...
        <table>
          <thead>
            <tr><th>Payment</th><th>Payee</th><th>Match</th><th>Score</th><th>Received</th><th>Status</th><th></th></tr>
          </thead>
          <tbody>
            {rows.map((r) => (
              <tr key={r.payment_id}>
